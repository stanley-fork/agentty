...
| `AGENTTY_RAG_ANN_DIM` | **Matryoshka** dimension truncation for the dense ANN index. `nomic-embed-text-v1.5` and the e5/BGE MRL variants pack the signal into the leading dims, so a prefix is itself a valid embedding. Set e.g. `256` (of 768) to build + search the HNSW graph on the leading 256 dims — ~2.3× faster graph walk + ⅓ the graph memory, with the full-dimension rerank stages recovering any precision. **Off by default** (0 = full width). A changed value auto-rebuilds the cached graph. |
| `AGENTTY_RAG_BINARY` | **Binary quantization** of the ANN walk (HuggingFace embedding quantization). Walks the HNSW graph on 1-bit-per-dim sign codes with popcount Hamming, then rescores the returned pool with the exact float cosine — binary recall, float precision (~0.97 recall@5 in-repo). ~2.5× faster walk; composes with `AGENTTY_RAG_ANN_DIM` (~3.4× combined). **Off by default**; set `1`/`true`/`on`. Toggling needs no cache rebuild (sign codes are derived from the stored vectors on load). |
| `AGENTTY_RAG_FUSION` | Hybrid fusion strategy. Default (`rrf`) is Reciprocal Rank Fusion — robust, rank-based. Set `rsf` (or `relative`/`score`) for **Relative Score Fusion**: min-max-normalize each list's raw scores to [0,1] then weighted-sum, preserving the score *magnitude* RRF discards. Wins when the per-list score distributions are calibrated + informative — A/B it on your corpus. |
| `AGENTTY_RAG_W_LEXICAL / AGENTTY_RAG_W_DENSE` | Hybrid fusion weights: how much the lexical (BM25) vs dense (embedding) ranked list counts in RRF/RSF. Defaults `1.0` / `1.3` (a modest dense tilt, the SOTA default for natural-language docs). A/B without a rebuild. |
...
| `AGENTTY_RAG_PROACTIVE_BUDGET_MS` | Fast-path **hedge** for the proactive pre-turn retrieval. The submit thread waits at most this long for the funnel (BM25 + dense query-embed + rerank/MMR); if it lands, grounding is injected inline with zero added latency. If it overruns on a large/slow corpus the submit thread never blocks — the turn enters its normal streaming state (status shows *retrieving context…*, the spinner is live, the UI never feels hung) and the stream launch is HELD behind a background retrieval that injects the block **same-turn** the moment it lands. Grounding is always for the question just asked; it is never dropped, never deferred to a later turn. Default `250`; `0` skips the inline hedge (always take the deferred path). |
